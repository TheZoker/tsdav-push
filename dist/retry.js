import { HttpStatusError, RetryExhaustedError, WebDavPushError, isRetryableStatus, } from "./errors.js";
...
function computeDelayMs(attempt, random, options) {
    const exp = Math.min(options.baseDelayMs * 2 ** (attempt - 1), options.maxDelayMs);
...
export async function withRetry(operation, options) {
    ...
    if (!lastError) {
        return {
            ok: false,
            error: new RetryExhaustedError("Retry exhausted without receiving a result.", {
                operation: "retry",
                retryable: false,
                headersSnapshot: {},
            }, attempts),
    ...
    return {
        ok: false,
        error: new RetryExhaustedError(`Retry exhausted after ${attempts} attempts.`, {
            operation: "retry",
